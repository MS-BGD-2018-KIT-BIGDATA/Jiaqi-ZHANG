...
def get_car_list(region, version, vendeur, car_list_url):
    ...
    [car_list_url.append(['https:' + x['href'], version, vendeur]) for x in car_list_href]
    return car_list_url
...
def get_car_specs(url, version, vendeur):
    ...
    # releaseDate = car_page.select("div.line:nth-of-type(1) span.value")[0] \
    #               .get_text()
    ...
    try:
        kilometrage = re.search("[0-9]+", kilometrage.replace(' ','')).group(0)
    except:
        kilometrage = ""
    ...
    try:
        tel_fixe = re.search("((\+|00)33\s?|0)[1](\s?\d{2}){4}", desc_parsed).group(0)
    except:
        tel_fixe = ""

    try:
        tel_port = re.search("((\+|00)33\s?|0)[67](\s?\d{2}){4}", desc_parsed).group(0)
    except:
        tel_port = ""
...
def get_cote_argus(version, year, kilometrage):
    url = "https://www.lacentrale.fr/cote-auto-renault-zoe-{version}-{year}.html"
...
if __name__ == "__main__":
    ...
    url = "https://www.leboncoin.fr/voitures/offres/{region}/?q={version}&brd=Renault&mdl=Zoe&f={vendeur}"
    ...
    # vendeurs = ["p"]
    car_list_url=[]
    [get_car_list(r, m , v, car_list_url) for r in regions for m in modeles for v in vendeurs]
    ...
    argus_list=[get_cote_argus(row.version,row.release_date,row.kilometrage) for row in df.itertuples()]
